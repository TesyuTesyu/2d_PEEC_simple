...
len_x=1 #横方向の長さ[m].縦は画像のアスペクトで決まる.
...
file_path=r"C:\Users\ntrt0\Documents\Python_Origin\PEEC_test2.png" #ファイルパス.

#電圧源の位置.
i_voltage=[2,49] #x方向の位置(?番目).
j_voltage=[2,30] #y方向の位置(?番目).
voltage=[0,1] #それぞれの電圧.
...
#デコード.
#v_ans=np.zeros([size_y,size_x], dtype=complex)
v_ans=np.full((size_y,size_x), np.nan)
k=0
for i in range(size_x):
    for j in range(size_y):
        if mat_shape[j,i]==1:
...
ix_ans=np.full((size_y,size_x-1), np.nan)
k_zero=k
mask_num=len(mat_true_x_Ix)
...
iy_ans=np.full((size_y-1,size_x), np.nan)
k_zero+=k+1
mask_num=len(mat_true_x_Iy)
for k in range(mask_num):
    i=mat_true_i_Iy[k]
