...
size_x=100 #横方向の分割数.縦は画像のアスペクトで決まる.

tickness=0.1e-3 #導体面の厚さ[m].

#matf=np.linspace(1e3,8e9,1)#周波数の配列.自由に変更.はじめは少ない数でテストした方がダメージが少ない.
matf=np.linspace(1e3,8e9,300)

#画像の設定
color_th=100 #白か黒かをこの閾値で判定.
file_path=r"???\PEEC_test4.png" #ファイルパス.
...
#電圧源の位置.
#例：.
i_voltage=[50] #x方向の位置(?番目).
j_voltage=[37] #y方向の位置(?番目).
voltage=[1] #それぞれの電圧.
...
make_matrix_v1.make_matrix(config_csv_file_path)
